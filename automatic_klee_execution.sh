...
SRC_CONVERTER_TOOL_DIR=$CONVERTER_TOOL_DIR/src
...
generate_main_module () {
  python3 generate_main_file.py $module
...
read_converter_errors () {
  ...
  for i in `ls $OUTPUT_KLEE_DIR/*.ktest`; do
    case=$(ls $i | awk -F. '{print $1}')
    #for j in $(seq 0 $num_objects); do
    #  ktest-tool $i | fgrep "object $j: name" > $case.txt
    #  ktest-tool $i | fgrep "object $j: int" >> $case.txt 
    #done
...
write_converter () {
  python3 generate_main_exec_file.py $module
  gnatmake -I$SRC_INCLUDE_DIR -I$SRC_CONVERTER_TOOL_DIR -D $LIB_CONVERTER_TOOL_DIR -o $BIN_CONVERTER_TOOL_DIR/$converter_tool $SRC_CONVERTER_TOOL_DIR/"$converter_tool.adb"
}
...
init () {
  ...
  # In development only remove klee directory
  #if [ -d $EXEC_DIR ]; then
  #  rm -rf $EXEC_DIR
  #fi
  ...
  ## Create again all directories
  mkdir -p $SRC_MAIN_DIR
  mkdir -p $SRC_INCLUDE_DIR
  mkdir -p $LIB_MAIN_DIR
  mkdir -p $LIB_INCLUDE_DIR
  # Output klee directory is created by Klee
  #mkdir -p $OUTPUT_KLEE_DIR
